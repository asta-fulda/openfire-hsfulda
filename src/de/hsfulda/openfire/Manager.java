...
public class Manager {
  ...
  private static final String SQL_USERS_SEARCH = "SELECT * FROM ofUser WHERE uid LIKE ? AND name LIKE ? AND email LIKE ?";
  ...
  static {
    ...
    Manager.USERS_SEARCH_FIELDS.put("User", 1);
    Manager.USERS_SEARCH_FIELDS.put("Name", 2);
    Manager.USERS_SEARCH_FIELDS.put("EMail", 3);
  ...
  /**
   * Connect to LDAP server using configured LDAP server URL.
   * 
   * @return the LDAP context
   * 
   * @throws NamingException
   */
  private LdapContext ldapConnect() throws NamingException {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        this.ldapAuthenticate(uid, password);
				...
				Log.debug("HS-FULDA: User '" + uid + "' authenticated agains LDAP.");
        
        // New user is successfully authenticated - populate database with new
        // user
        this.createUser(uid);
				Log.debug("HS-FULDA: User '" + uid + "' created.");
        
        try {
          // Load created user
          User user = this.loadUser(username);
				  Log.debug("HS-FULDA: User '" + uid + "' loaded.");
          
          // Cache created user
          CacheFactory.createCache("User").put(username, user);
          
          // Notify about new user
          Map<String, Object> parameters = Collections.emptyMap();
          UserEventDispatcher.dispatchEvent(user, UserEventDispatcher.EventType.user_created, parameters);
   ...
   * @throws NamingException
   */
  private SearchResult ldapSearchUser(final LdapContext ldapContext, final String username) throws NamingException {
    ...
    NamingEnumeration<SearchResult> answer = ldapContext.search("", "(cn={0})", new String[] { username }, searchControls);
  ...
  private void ldapAuthenticate(final String username, final String password) throws UnauthorizedException {
    try {
      ...
      LdapContext ldapContext = this.ldapConnect();
      
      // Search for user using user name
      SearchResult searchResult = this.ldapSearchUser(ldapContext, username);
  ...
  private void createUser(final String username) throws UnauthorizedException {
    ...
    try {
      ...
      // Set password field to NULL, to indicate the referencing to LDAP server
      statement.setNull(2, Types.VARCHAR);
      
      // Copy permanent user information
      User user = this.ldapLoadUser(username);
  ...
  private User ldapLoadUser(final String uid) throws NamingException {
    ...
    SearchResult searchResult = this.ldapSearchUser(ldapContext, uid);
    ...
    if (searchResult != null) {
      ...
      Attributes attributes = ldapContext.getAttributes(searchResult.getName(), new String[] { "givenName", "sn", "mail", "createTimestamp", "modifyTimestamp" });
      ...
      user = new User(uid, name.toString(), email != null ? email.toString() : "", creationDate, modificationDate);
  ...
  public Element loadVCard(final String username) throws UserNotFoundException {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        String avatar_mime = rs.getString("avatar_mime");
        
        vCard = DocumentHelper.createElement("vCard");
        
        Element eUsername = DocumentHelper.createElement("NICKNAME");
        eUsername.setText(username);
        vCard.add(eUsername);
        
        if (name != null) {
          Element eName = DocumentHelper.createElement("FN");
          eName.setText(name);
          vCard.add(eName);
        }
        ...
        if (email != null) {
          ...
          eEmail.add(DocumentHelper.createComment("INTERNET"));
          vCard.add(eEmail);
          
          Element eEmailInternet = DocumentHelper.createElement("USERID");
          eEmailInternet.setText(email);
          vCard.add(eEmail);
        }
        
        if (avatar != null) {
          Element eAvatar = DocumentHelper.createElement("PHOTO");
          vCard.add(eAvatar);
          
          Element eAvatarValue = DocumentHelper.createElement("BINVAL");
          eAvatarValue.setText(avatar);
          vCard.add(eAvatarValue);
          
          Element eAvatarMime = DocumentHelper.createElement("TYPE");
          eAvatarMime.setText(avatar_mime);
          vCard.add(eAvatarMime);
        }
      }
